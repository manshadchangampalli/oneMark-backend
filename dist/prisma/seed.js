...
const prisma = new prisma_1.PrismaClient({ adapter: new adapter_pg_1.PrismaPg(pool) });
const EXAMS = [
...
async function main() {
    ...
    for (const q of SAMPLE_QUESTIONS) {
        ...
        if (!subject) {
            console.warn(`  subject ${q.subjectCode} not found, skip`);
            continue;
        }
        const topic = await prisma.topic.findUnique({
        ...
        if (!topic) {
            console.warn(`  topic ${q.topicCode} not found, skip`);
            continue;
        }
